...
#    os_vol_attached_in_project_trimmed=$(echo $os_vol_attached_in_project | sed 's/"//g' | sed "s/'/\"/g" | sed 's/[][]//g' | jq .server_id)
    os_vol_attached_in_project_trimmed=$(eval echo $os_vol_attached_in_project | cut -c173-208)
