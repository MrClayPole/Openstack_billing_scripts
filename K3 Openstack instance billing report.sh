...
do
  ...
  do
    os_instace_cpu_hours=$(openstack metric measures show -r $os_vm_id_in_project vcpus --granularity 300 --resample 3600 -f value -c value --start $start_date --stop $end_date | paste -sd+ - | bc)
    os_instace_memory_hours=$(openstack metric measures show -r $os_vm_id_in_project memory --granularity 300 --resample 3600 -f value -c value --start $start_date --stop $end_date | paste -sd+ - | bc)
